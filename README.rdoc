...
The only (minor) limitation of Executable, as opposed to other light-weight
command-line parsers is the need to use an <code>=</code> sign when specifying
options that take an argument, e.g.

  x.execute_command("foo --name=example")
